...
UINT32 OV5647MIPISetMaxFrameRate(UINT16 u2FrameRate)
{
    ...
    if((CurrentScenarioId==MSDK_SCENARIO_ID_CAMERA_ZSD)||(CurrentScenarioId==MSDK_SCENARIO_ID_CAMERA_CAPTURE_JPEG)) {
...
UINT32 OV5647MIPISetVideoMode(UINT16 u2FrameRate)
{
	...
	if (u2FrameRate > 30)
    {
        SENSORDB("[OV5647MIPISetVideoMode]u2FrameRate=%d is invalide!",u2FrameRate);
        u2FrameRate = 30;
        OV5647MIPI_sensor.NightMode = KAL_FALSE;
    }
