...
#define APDS9930_CMM_ATIME_MS (2.72 * (0xff - 0xF6 + 1))
#define APDS9930_CMM_AGAINx 120
#define APDS9930_CMM_GA 1
...
*****************************************************************************/
static int luxcalculation(struct i2c_client *client,int c0data, int c1data)
{
	int luxValue = 0;
	int control_reg = 0;
	int cp_temp;
	...
	control_reg  = i2c_smbus_read_byte_data(client, APDS9930_CMM_CONTROL);
	control_reg &=0x03;

	cp_temp   = (againx_table[control_reg] * (272 * (0xff - 0xF6 + 1))) / (APDS9930_CMM_GA * 24);
...
{	 
        ...
        else
	{
       ...
       #endif
        *data = luxcalculation(client,c0_nf, c1_nf);
